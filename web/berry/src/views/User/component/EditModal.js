...
const validationSchema = Yup.object().shape({
  ...
  group: Yup.string().when('is_edit', {
    is: false,
    then: Yup.string().required('User组 cannot be empty'),
    otherwise: Yup.string()
  }),
  quota: Yup.number().when('is_edit', {
    is: false,
    then: Yup.number().min(0, 'Quota 不能小于 0'),
...
const EditModal = ({ open, userId, onCancel, onOk }) => {
  ...
  const submit = async (values, { setErrors, setStatus, setSubmitting }) => {
    ...
    if (success) {
      if (values.is_edit) {
        showSuccess('Userupdated successfully！');
      } else {
        showSuccess('Usercreated successfully！');
      }
      setSubmitting(false);
          ...
          {({ errors, handleBlur, handleChange, handleSubmit, touched, values, isSubmitting }) => (
            <form noValidate onSubmit={handleSubmit}>
              <FormControl fullWidth error={Boolean(touched.username && errors.username)} sx={{ ...theme.typography.otherInput }}>
                <InputLabel htmlFor="channel-username-label">User名</InputLabel>
                <OutlinedInput
              ...
              <FormControl fullWidth error={Boolean(touched.display_name && errors.display_name)} sx={{ ...theme.typography.otherInput }}>
                <InputLabel htmlFor="channel-display_name-label">显示Name</InputLabel>
                <OutlinedInput
